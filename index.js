...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Pegasus\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5562995223384:+55 47992091566\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nBem vindo ao grupo *${mdata.subject}*\n\nEspero que goste do grupo e seja ativo 💕️`
				...
				teks = `Tchau @${num.split('@')[0]} Foi tarde kkkk👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD PEGASUS BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Pegasus? Este é um recurso especial para o Pegasus ❌',
					ownerB: '[❗] Pegasus? Este é um recurso especial para o Pegasus ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de Pegasus. Registre-se para fazer amizade com o bot Pegasus por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar Pegasus|17\n\n──「 PEGASUS BOT 」──`,
			...
			const ownerNumber = ["5562995223384@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5562995223384@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5562995223384@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5562995223384@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5562995223384@s.whatsapp.net","558296156440@s.whatsapp.net"] 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("bot")){
            ...
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
				...
				if (messagesC.includes("pegasus")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/pegasusf.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS PEGASUS BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Pegasus domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o Pegasus ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT PEGASUS ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *PEGASUS* 」*────*\n\n*_SE QUER SER ADMIN DO BOT PEGASUS_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5562995223384/*╰──────*「 * PEGASUS* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO PEGASUS BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do Pegasus:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
